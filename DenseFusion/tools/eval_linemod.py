...
for i, data in enumerate(testdataloader, 0):
    points, choose, img, target, model_points, idx,path = data
    ...
    if dis < diameter[idx[0].item()]:
        ...
        ##### for visualization
                ...
                [0, cam_fy, cam_cy],
                [0, 0, 1]])
                image_points, _ = cv2.projectPoints(pred, np.eye(3), np.array((0,0,0)).astype(float), K, None)
                ...
                for pointt in image_points:
                        cv2.circle(img_rgb, tuple(pointt), 2, (0, 255, 0), -1)

                cv2.imshow("cloud",img_rgb)
                ...
                z_max = np.max(pred[:,2])
                z_min = np.min(pred[:,2])
                pred_box = np.array([[x_min, y_min, z_min],
                                        [x_max, y_min, z_min],
                                        [x_max, y_max, z_min],
                                        [x_min, y_max, z_min],
                ...
                img_box=draw_3d_bbox(pred_box_pixel,img_box)
