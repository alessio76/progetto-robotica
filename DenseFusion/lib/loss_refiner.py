...
def loss_calculation(pred_r, pred_t, target, model_points, idx, points, num_point_mesh, sym_list):
    ...
    if idx[0].item() in sym_list:
        ...
        inds = knn.apply(target.unsqueeze(0), pred.unsqueeze(0),1)
...
"""class Loss_refine(_Loss):
    ...
    def forward(self,pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine):
        loss, dis, new_points, new_target = loss_calculation(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, self.num_pt_mesh,self.sym_list)
        return loss,dis,new_points,new_target"""
class Loss_refine(_Loss):

    def __init__(self, num_points_mesh, sym_list):
        super(Loss_refine, self).__init__()
        self.num_pt_mesh = num_points_mesh
        self.sym_list = sym_list
		
    def forward(self,pred_r, pred_t, target, model_points, idx, points):
        dis, new_points, new_target = loss_calculation(pred_r, pred_t, target, model_points, idx, points, self.num_pt_mesh,self.sym_list)
        return dis,new_points,new_target
